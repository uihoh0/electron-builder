...
import { configureRequestUrl, CancellationToken, GenericServerOptions, HttpError, newError, UpdateInfo, WindowsUpdateInfo } from "builder-util-runtime"
...
let appUpdateInfo: any
function getAppUpdateInfoPath() {
  return app.isPackaged ? path.join(process.resourcesPath!!, "app-update.yml") : path.join(app.getAppPath(), "dev-app-update.yml")
}
async function getAppUpdateInfo(): Promise<any> {
  if (appUpdateInfo !== undefined) {
    return appUpdateInfo
  }
  const configPath = getAppUpdateInfoPath()
  const config: YmlConf = safeLoad(await readFile(configPath, "utf-8"))
  appUpdateInfo = config ? config : {}
  return appUpdateInfo
}
...
export class GenericProvider extends Provider<UpdateInfo> {
  ...
  async getLatestVersion(): Promise<UpdateInfo> {
    ...
    const _appInfo = await getAppUpdateInfo()
    const dnsResolver = await dnsResolverManager({
      ..._appInfo.httpdns,
      httpExecutor: async (url: string): Promise<any> => {
        const httpExecutor = new ElectronHttpExecutor((authInfo: any, callback: any) => callback)
        const opts = {}
        configureRequestUrl(new URL(url), opts)
        let result
        result = await httpExecutor.request(opts)
